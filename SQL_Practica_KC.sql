...
create table vehiculos.marca(
	...
	constraint grupo_fk foreign key (id_grupo) references vehiculos.grupo(id_grupo)
);
...
create table vehiculos.modelo(
	...
	constraint marca_fk foreign key (id_marca) references vehiculos.marca(id_marca)
);
...
create table vehiculos.vehiculo(
	...
	constraint modelo_fk foreign key (id_modelo) references vehiculos.modelo(id_modelo),
	constraint color_fk foreign key (id_color) references vehiculos.color(id_color)
);
...
create table vehiculos.poliza(
	id_poliza serial primary key,
	num_poliza int not null,
	id_aseguradora int not null,
	id_vehiculo int not null,
	fecha_poliza date not null,
	constraint aseguradora_fk foreign key (id_aseguradora) references vehiculos.aseguradora(id_aseguradora),
	constraint vehiculo_fk foreign key (id_vehiculo) references vehiculos.vehiculo(id)
);
...
create table vehiculos.revision(
	...
	constraint moneda_fk foreign key (id_moneda) references vehiculos.moneda(id_moneda),
	constraint vehiculo2_fk foreign key (id_vehiculo) references vehiculos.vehiculo(id)

);

CREATE TABLE vehiculos.coches (
...
INSERT INTO vehiculos.coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
insert into vehiculos.grupo (nombre)
select grupo from vehiculos.coches group by grupo order by grupo;

insert into vehiculos.marca (nombre,id_grupo)
select coches.marca,grupo.id_grupo from vehiculos.coches 
inner join vehiculos.grupo on grupo.nombre = coches.grupo
group by coches.marca,grupo.id_grupo;


insert into vehiculos.modelo  (nombre,id_marca)
select coches.modelo,marca.id_marca from vehiculos.coches 
inner join vehiculos.marca on marca.nombre= coches.marca
group by coches.modelo,marca.id_marca order by marca.id_marca;


insert into vehiculos.color (nombre) select color from vehiculos.coches group by color order by color;

insert into vehiculos.vehiculo  (matricula,id_modelo,fecha_compra,km,id_color) 
select coches.matricula,modelo.id_modelo,coches.fecha_compra,coches.kms_totales,color.id_color
from vehiculos.coches 
inner join vehiculos.modelo on modelo.nombre = coches.modelo
inner join vehiculos.color on color.nombre = coches.color
group by coches.matricula,modelo.id_modelo,coches.fecha_compra,coches.kms_totales,color.id_color
order by coches.fecha_compra;

insert into vehiculos.moneda (nombre,cod_iso) select moneda,
				case when moneda = 'Dólar' then 'USD' 
					 when moneda = 'Euro' then 'EUR'
					 when moneda = 'Peso Mexicano' then 'MXN'
					 when moneda = 'Peso Colombiano' then 'COP'
					 else 'N/A' end as cod_iso from vehiculos.coches group by moneda order by moneda;
					 
					
					
insert into vehiculos.aseguradora (nombre) select aseguradora from vehiculos.coches group by aseguradora order by aseguradora;

insert into vehiculos.poliza (num_poliza,id_aseguradora,id_vehiculo,fecha_poliza)
select coches.n_poliza,aseguradora.id_aseguradora,vehiculo.id,coches.fecha_alta_seguro 
from vehiculos.coches 
inner join vehiculos.vehiculo on vehiculo.matricula = coches.matricula 
inner join vehiculos.aseguradora on aseguradora.nombre = coches.aseguradora
group by coches.n_poliza,aseguradora.id_aseguradora,vehiculo.id,coches.fecha_alta_seguro
order by vehiculo.id,coches.fecha_alta_seguro ;

insert into vehiculos.revision (id_vehiculo,fecha_revision,km_revision,importe_revision,id_moneda) 
select vehiculo.id,coches.fecha_revision,coches.kms_revision,coches.importe_revision,moneda.id_moneda
from vehiculos.coches 
inner join  vehiculos.vehiculo  on vehiculo.matricula = coches.matricula 
inner join vehiculos.moneda on moneda.nombre = coches.moneda

group by vehiculo.id,coches.fecha_revision,coches.kms_revision,coches.importe_revision,moneda.id_moneda
order by vehiculo.id,coches.fecha_revision;


drop table vehiculos.coches ;
...
select 	grupo.nombre as grupo,
		...
		vehiculo.km as total_km,
		poliza.num_poliza as numero_poliza,
		aseguradora.nombre as aseguradora
		from vehiculos.vehiculo
		inner join vehiculos.color on vehiculo.id_color = color.id_color
		inner join vehiculos.modelo on vehiculo.id_modelo = modelo.id_modelo
		inner join vehiculos.marca on modelo.id_marca = marca.id_marca
		inner join vehiculos.grupo on marca.id_grupo = grupo.id_grupo
		inner join (select max(fecha_poliza) as fecha_poliza, id_vehiculo from vehiculos.poliza group by id_vehiculo ) as polizas on vehiculo.id = polizas.id_vehiculo
		inner join vehiculos.poliza on polizas.id_vehiculo = poliza.id_vehiculo and polizas.fecha_poliza = poliza.fecha_poliza
		inner join vehiculos.aseguradora on poliza.id_aseguradora = aseguradora.id_aseguradora;
